...
function migrate(db) {
  ...
  `);
  // Defensive migration for DBs created before torrent_b64 existed.
  try { db.exec('ALTER TABLE torrents ADD COLUMN torrent_b64 TEXT'); } catch { /* already there */ }
}

export function getTorrentFile(infohash) {
  const row = getDb().prepare('SELECT torrent_b64 FROM torrents WHERE infohash = ?').get(infohash);
  return row?.torrent_b64 ? Buffer.from(row.torrent_b64, 'base64') : null;
}

export function putBlob(path, buf, contentType) {
  getDb().prepare('INSERT OR REPLACE INTO blobs (path, content_b64, content_type) VALUES (?, ?, ?)')
    .run(path, buf.toString('base64'), contentType);
}

export function getBlob(path) {
  const row = getDb().prepare('SELECT content_b64, content_type FROM blobs WHERE path = ?').get(path);
  return row ? { buf: Buffer.from(row.content_b64, 'base64'), type: row.content_type } : null;
}
...
export function insertTorrent(t) {
  ...
  db.prepare(`
    INSERT INTO torrents (infohash, name, category, size_bytes, seeds, leechers,
      uploader_id, uploaded_at, license, source_url, description, verified, files_json, webseeds_json, torrent_b64)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    ...
    t.verified ?? 0, t.files_json ?? null, t.webseeds_json ?? null, t.torrent_b64 ?? null,
  );
}
